...
class Client2ClientHandler(object):
    ...
    def __call__(self, protocol, msg):
        ...
        layers.
        """
        packetTrace(logger, msg, "Begin processing c2c message")
        msgObj = msg.data()
        ...
        """
        factory, openConnections = self.__portToProtocol[dstPort]
        peerHost, peerPort = msgObj.srcAddress, msgObj.srcPort
        connectionKey = (peerHost, peerPort)
        
        """ Check for an existing connection """
        if not openConnections.has_key(connectionKey):
            """ No existing connection. Check for a factory. """
            if not factory:
                """ If factory is None, this means that we are not listening on this port..."""
                packetTrace(logger, msg, "An open out-going connection is open on this port (%d). Packet dropped." % dstPort)
                return
            
            """ We have a factory. Create a new connection, protocol, and transport """
            ...
            prot = factory.buildProtocol(thisAddressPair)
            prot.makeConnection(ClientApplicationTransport(protocol.transport, thisAddressPair, peerAddressPair, self.__closer))
            openConnections[connectionKey] = prot
            packetTrace(logger, msg, "New connection created from %s:%d to %s:%d" % (peerHost, peerPort, dstAddress, dstPort))
        else:
            prot = openConnections[connectionKey]
        ...
        if hasattr(msgObj, "ID"):
            ...
            if self.__frags[msgObj.ID]["last"] != None:
                ...
                if packetsReceived == expectedPackets:
                    ...
                    try:
                        prot.dataReceived(reassembled)
                    except Exception, e:
                        protocol.reportException(e)
                        protocol.callLater(1.0, lambda: self.__safeClose(prot))
                    del self.__frags[msgObj.ID]
        else:
            trueMessage = msgObj.clientPacket
            try:
                prot.dataReceived(trueMessage)
            except Exception, e:
                protocol.reportException(e)
                protocol.callLater(1.0, lambda: self.__safeClose(prot))